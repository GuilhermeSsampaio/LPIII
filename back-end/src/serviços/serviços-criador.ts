...
export default class ServiçosCriador {
  ...
  static async cadastrarCriador(request, response) {
    try {
      const { usuário_info, pais_origem, ano_fundacao, estilo_cerveja_especializado } =
        request.body;
  ...
  static async atualizarCriador(request, response) {
    try {
      const { cpf, pais_origem, ano_fundacao, estilo_cerveja_especializado } = request.body;
  ...
  static async buscarTodasCervejasArtesanais(request, response) {
    try {
